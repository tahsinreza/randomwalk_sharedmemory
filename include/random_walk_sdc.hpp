...
template <typename Vertex, typename Edge, typename VertexList, 
  typename VertexDataList, typename EdgeList, typename Uint>
class random_walk {
  public:
    static const uint64_t RED = 756880090378066622;
    static const uint64_t GREEN = 519353457214661108;   
    ...
    void do_random_walk(VertexList& vertices, VertexList& vertex_degree,
      VertexDataList& vertex_data, EdgeList& edges, 
      std::string walks_output_filepath) {
      ...
      for(size_t walker_ID = 0; walker_ID < walker_count; walker_ID++) {
        ...
        bool is_valid_source_vertex = false; 
        bool is_valid_walk = false;     

        // only start walker from a blue (type) vertex 
        if (vertex_data[current_vertex] == RED  || 
          vertex_data[current_vertex] == GREEN) {
          walker_alive = false;
          walker_memory_per_thread[thread_ID].clear();
          current_walker_hop_count = walker_hop_count;
          trials = max_trials;  
        } else {
          is_valid_source_vertex = true;
        }            
 
        do {
          if (vertex_degree[current_vertex] > 0 && is_valid_source_vertex) {
            ...
            // check if the current vertex has the same label as the source
            if (vertex_data[current_vertex] != RED  &&
              vertex_data[current_vertex] != GREEN  &&
              vertex_data[current_vertex] == vertex_data[walker_memory_per_thread[thread_ID][0]]) {
              walker_alive = false;              
              current_walker_hop_count = walker_hop_count;
              trials = max_trials;
              is_valid_walk = true;   
        ...
        // source and sink cannot be the same
        if (walker_memory_per_thread[thread_ID][0] == 
          walker_memory_per_thread[thread_ID][walker_memory_per_thread[thread_ID].size() - 1]) {
          is_valid_walk = false;
        }     

        //output_walks(walks_output_filepath, walker_ID, thread_ID);
        if (is_valid_source_vertex && is_valid_walk) { 
          output_walks(walks_output_file_per_thread[thread_ID], walker_ID, thread_ID, vertex_data);
        }
      } // for 
  ...
  private:
    ...
    void output_walks(std::ofstream& walks_output_file, size_t walker_ID, 
      size_t thread_ID, VertexDataList& vertex_data) {
      //std::string walks_output_filename = walks_output_filepath + 
      ...
      for (size_t v = 0; v < walker_memory_per_thread[thread_ID].size(); v++) {
        auto _vertex = walker_memory_per_thread[thread_ID][v];
        std::string _vertex_data_str = "";
        if (vertex_data[_vertex] == RED) {
          _vertex_data_str = "R";  
        } else if (vertex_data[_vertex] == GREEN) {
          _vertex_data_str = "G";
        } else {
          _vertex_data_str = std::to_string(vertex_data[_vertex]);
        }
